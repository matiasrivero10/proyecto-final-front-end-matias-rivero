        ...
        fetch(config.apiURL+"clients/"+config.operatorId, requestOptions).then((response) => {
            ...
            if(clientList.length === 0){
                ...
                rowData = clientList.map(p => {
                    let button;
                    if(p.active){
                        button = <button className="btn btn-secondary" onClick={() => disable(p)}><i className="fas fa-eye-slash"></i> Desvincular</button>;
                    } else {
                        button = <button className="btn btn-primary" onClick={() => enable(p)}><i className="fas fa-eye"></i> Revincular</button>;
                    }
